<template>
  ...
  <div class="box box-shadow">
    ...
    <el-dialog v-model="ui.dialogVisible" width="40%" class="dialog" :title="ui.addRecord?'Add topping':'Update topping'">
      <el-form :model="formData" label-width="auto">
        <el-input v-model="topping_id" class="d-none" />
...
const handleConfirm = async () => {
  ...
  if (ui.value.addRecord) {
    for (const file of fileList.value) {
      try {
        formData.value.image = await uploadFileToFirebaseAndGetURL(file.raw, 'topping');
      } catch (error) {
...
const handleFileInputChange = async (event: Event, fileToEdit: File) => {
  const input = event.target;
  if (input.files && input.files.length > 0) {
    const newFile = input.files[0];
    const downloadURL = await uploadFileToFirebaseAndGetURL(newFile, 'topping');
